...
def d2_process_json(data: dict) -> Tuple[List[ArgumentCodeBlock], List[ClassCodeBlock]]:
    # d2 = a dict with 1 dict inside of it
    fields: List[ArgumentCodeBlock] = []
    child_classes = []
    for index, (key, value) in enumerate(data.items()):
        if isinstance(value, dict):
            child_object_data = d2_process_json(value)
            ...
            arg = ArgumentCodeBlock(key, child_object.name, field=None)
        else:
            arg = ArgumentCodeBlock(key, type_hint=type(value), field=None)
        fields.append(arg)

    return fields, child_classes


def json_to_code(data: dict) -> FileCodeBlock:
    args = d2_process_json(data)
